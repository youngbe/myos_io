...
void __attribute__((noinline)) set_thread_schedulable(struct Thread *const new_thread, const bool is_sti)
{
    if (new_thread == NULL)
    ...
    al_append_inline(&schedubable_threads, &new_thread->al_node, false);
    atomic_fetch_add_explicit(&schedulable_threads_num, 1, memory_order_acq_rel);
    if (atomic_load_explicit(&idle_cores_num, memory_order_relaxed) > 0)
