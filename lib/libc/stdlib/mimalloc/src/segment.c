...
static size_t mi_segment_calculate_slices(size_t required, size_t*, size_t* info_slices) {
  size_t isize = _mi_align_up(sizeof(mi_segment_t), MI_SEGMENT_SLICE_SIZE);
  if (info_slices != NULL) *info_slices = isize / MI_SEGMENT_SLICE_SIZE;

  const size_t page_size = _mi_os_page_size();

  mi_assert_internal(MI_SEGMENT_SIZE > isize && MI_SEGMENT_SIZE % page_size == 0 && MI_SEGMENT_SIZE % MI_SEGMENT_SLICE_SIZE == 0);
  mi_assert_internal(page_size % MI_SEGMENT_SLICE_SIZE == 0 || MI_SEGMENT_SLICE_SIZE % page_size == 0);
  const size_t segment_size = (required == 0 ? MI_SEGMENT_SIZE : _mi_align_up(required + isize, page_size > MI_SEGMENT_SLICE_SIZE ? page_size : MI_SEGMENT_SLICE_SIZE));  
...
static void mi_segment_delayed_decommit(mi_segment_t* segment, bool, mi_stats_t* stats) {
